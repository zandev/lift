...
class PostalCodeField[OwnerType <: Record[OwnerType]](rec: OwnerType, country: CountryField[OwnerType]) extends StringField(rec, 32) {
  ...
  def validatePostalCode(in : Box[String]): List[FieldError] = country.value match {
    case Countries.USA       => in.map(valRegex(RegexPattern.compile("[0-9]{5}(\\-[0-9]{4})?"), S.??("invalid.zip.code"))) openOr Nil
    case Countries.Sweden    => in.map(valRegex(RegexPattern.compile("[0-9]{3}[ ]?[0-9]{2}"), S.??("invalid.postal.code"))) openOr Nil
    case Countries.Australia => in.map(valRegex(RegexPattern.compile("(0?|[1-9])[0-9]{3}"), S.??("invalid.postal.code"))) openOr Nil
    case Countries.Canada    => in.map(valRegex(RegexPattern.compile("[A-Z][0-9][A-Z][ ][0-9][A-Z][0-9]"), S.??("invalid.postal.code"))) openOr Nil
    case _ => genericCheck(in)
  }

  override def validators = validatePostalCode _ :: Nil
