...
package mongotestrecords {
  ...
  case class JsonDoc(id: UUID, name: String) extends JsonObject[JsonDoc] {
    def meta = JsonDoc
  }
...
object MongoRecordExamples extends Specification {
  ...
  "List example" in {
    ...
    ld1.jsonobjlist.set(List(JsonDoc(UUID.randomUUID, "jsondoc1"), JsonDoc(UUID.randomUUID, "jsondoc2")))
